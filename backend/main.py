...
app = FastAPI(title="PaperPulse API — AI-Based Intelligent Research Paper Recommendation & Literature Survey Assistant")

# Import and include Phase 2 Recommendation Router
try:
    from backend.recommendation import recommendation_router
    ...
    try:
        ...
        logger.warning(f"Could not load recommendation_router: {e}")
...
@app.get("/")
async def read_root():
    return {"message": "AI-Based Intelligent Research Paper Recommendation and Literature Survey Assistant API", "status": "running"}
