...
const STORAGE_KEY = 'paperpulse_session';
...
function sanitizeSession(session) {
  ...
  return {
    ...
    savedPapers: session.savedPapers || [],
  };
}
...
export function useSessionPersistence(initialSession) {
  const [session, setSessionState] = useState(() => {
    ...
    return { ...initialSession, savedPapers: [] };
  });
    ...
    const timer = setTimeout(() => {
      try {
        ...
        try {
          ...
          const minimal = {
            ...session,
            docs: (session.docs || []).map(d => ({ ...d, fullText: '' })),
            savedPapers: session.savedPapers || [],
          };
  ...
  const clearSession = useCallback(() => {
    localStorage.removeItem(STORAGE_KEY);
    setSessionState({ ...initialSession, savedPapers: [] });
  }, [initialSession]);
